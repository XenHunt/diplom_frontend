...
export class MediaService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.filter = new BehaviorSubject<[FilterType, FilterSort]>([
      FilterType.All,
      FilterSort.NameAscended,
    ]);
  ...
  private getMedias() {
    this.http
      .get<Media[]>(`${environmet.apiUrl}/all_media`)
      .pipe(map((medias) => Array.from(medias)))
      .subscribe({
        next: (medias) => {
  ...
  public uploadMedia(file: File, name: string) {
    ...
    return this.http
      .post<{ message: string; model: Media }>(
        `${environmet.apiUrl}/upload`,
        formData,
        {
        ...
        map((res) => {
          if (res instanceof HttpResponse)
            if (res.type === HttpEventType.Response) {
              // Добавим новую медиа в лист
              const media = this.mediaList$.getValue();
              const body = res.body;
              if (body != null)
                if (media == null) this.mediaList$.next([body.model]);
                else {
                  media.push(body.model);
                  this.mediaList$.next(media);
                }
            }
          return res;
        }),
  ...
  public checkMediaStatusOneTime(media: Media) {
    return this.http.get<any>(
      `${environmet.originalUrl}/${media.type}_${media.id}/status`,
      { withCredentials: true },
    );
  }
  ...
  public getEditIds(frame_number: number) {
    ...
    if (media.type === MediaType.Video)
      return this.http.get<Array<number>>(
        `${environmet.originalUrl}/video-edit/${media.id}/${frame_number}/get-ids`,
        { withCredentials: true },
      );
    return this.http.get<Array<number>>(
      `${environmet.originalUrl}/image-edit/${media.id}/get-ids`,
      { withCredentials: true },
    );
  }
  ...
  public getEditFrames() {
    const media = this.selectedMedia$.getValue();
    if (media === null || media.type === MediaType.Image) return null;
    return this.http.get<any>(
      `${environmet.originalUrl}/video-edit/${media.id}/get-frames`,
      { withCredentials: true },
    );
  }
  ...
  public getEditCar(
    ...
    if (media.type === MediaType.Video) {
      let req = new HttpRequest(
        'POST',
        `${environmet.originalUrl}/video-edit/${media.id}/${frame_number}/${car_id}`,
        formData,
    ...
    let req = new HttpRequest(
      'POST',
      `${environmet.originalUrl}/image-edit/${media.id}/${car_id}`,
      formData,
      { responseType: 'blob', withCredentials: true },
    );
    return this.http.request<Blob>(req).pipe(
  ...
  public getPlateNumber(car_id: number) {
    ...
    if (model.type === MediaType.Video)
      return this.http.get<any>(
        `${environmet.originalUrl}/video-edit/${model.id}/${car_id}/get-plate-number`,
        { withCredentials: true },
      );
    return this.http.get<any>(
      `${environmet.originalUrl}/image-edit/${model.id}/${car_id}/get-plate-number`,
      { withCredentials: true },
    );
  }
  public updateMedia(car_id: number, plate_number: string) {
    ...
    if (media.type === MediaType.Video)
      return this.http.post<any>(
        `${environmet.originalUrl}/video-edit/${media.id}/${car_id}/update`,
        formData,
        { withCredentials: true },
      );
    return this.http.post<any>(
      `${environmet.originalUrl}/image-edit/${media.id}/${car_id}/update`,
      formData,
      { withCredentials: true },
    );
  }

  public getSearchFrames(pattern: string) {
    console.log(pattern);
    const model = this.selectedMedia$.getValue();
    if (model === null) return;
    const formData = new FormData();
    formData.append('searchPattern', pattern);
    return this.http
      .post<any>(
        `${environmet.originalUrl}/video/${model.id}/search`,
        formData,
        { withCredentials: true },
      )
      .pipe(map((res: { cars: Array<SearchElement> }) => res.cars));
  }
}
